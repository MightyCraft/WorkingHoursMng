...
class MemberType
{
	...
	/**
	 * 社員タイプマスタのデータをID指定で取得
	 *
	 * @param	integer	$member_type_id
	 * @param	boolean	$delete_flg
	 * @return array 全データ
	 */
	function getDataById($member_type_id,$delete_flg=false)
	{
		$return = array();
		if (!empty($member_type_id))
		{
			$where_columns['id'] = (int)$member_type_id;
			if (!$delete_flg)
			{
				$where_columns['delete_flg'] = 0;
			}
	 ...
	 * @return	array 登録結果
	 */
	function insertMemberType($regist_data)
	{
		$response=null;
		$insert_id=null;
	 ...
	 * @return	array	更新結果
	 */
	function updateMemberType($id,$update_columns)
	{
